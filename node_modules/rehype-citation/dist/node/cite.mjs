...
function getNodeRequestOptions$1(request) {
  ...
  const headers = new Headers$2(request[INTERNALS$2$1].headers);
...
function fetch$3(url, opts) {
  ...
  Body$2.Promise = fetch$3.Promise;
...
function fetch$2(resource, init) {
  const request = [];
  if (resource instanceof fetch$2.Request) {
  ...
  if ('headers' in response[1]) {
    return shared.deserializeResponse(fetch$2, ...response);
  } else {
    throw shared.deserializeError(fetch$2, ...response);
  }
}
...
class Body$1 {
  ...
  json() {
    ...
    try {
      ...
      throw new fetch$2.FetchError(`invalid json response body at ${this.url} reason: ${err.message}`, 'invalid-json');
    }
  }
  buffer() {
    checkBody(this);
  ...
  textConverted() {
    throw new fetch$2.FetchError('textConverted not implemented');
  }
}
...
function defineBodyError(body, error) {
  Object.defineProperty(body, _bodyError, {
    value: shared.deserializeError(fetch$2, ...error),
    enumerable: false
  });
}
...
fetch$2.Headers = lib$1.Headers;
fetch$2.FetchError = lib$1.FetchError;
fetch$2.Request = SyncRequest;
fetch$2.Response = SyncResponse;
...
const mathCommands = unicode$2.mathCommands;
...
const lexer$1 = moo.states({
  ...
  entry: _objectSpread$4(_objectSpread$4({}, whitespace), {}, {
    otherEntryType: {
  ...
  otherEntryContents: _objectSpread$4(_objectSpread$4({}, whitespace), {}, {
    lbrace: {
      match: /[{(]/,
      next: 'fields'
    }
  }),
  dataEntryContents: _objectSpread$4(_objectSpread$4({}, whitespace), {}, {
    lbrace: {
      match: /[{(]/,
      next: 'dataEntryContents'
    },
  ...
  fields: _objectSpread$4(_objectSpread$4({}, whitespace), {}, {
    identifier,
    number: /-?\d+/,
    hash: '#',
    equals: '=',
...
function crossref(target, entry, registry) {
  if (entry.crossref in registry) {
    ...
    const data = _objectSpread$3({}, crossref(parent.type, parent.properties, registry));
    delete data.ids;
...
function _parse(input, spec) {
  ...
  return input.map(({
    ...
    label,
    properties
  }) => spec.convertToTarget(_objectSpread$2({
    [TYPE]: type,
    [LABEL]: label
...
function _format(input, spec) {
  return input.map(entry => {
    const _spec$convertToSource = spec.convertToSource(entry),
      ...
      properties = _objectWithoutProperties(_spec$convertToSource, [TYPE, LABEL].map(_toPropertyKey$2));
    return {
      type,
      label,
      properties
...
const lexer = moo.states({
  stringLiteral: _objectSpread$1(_objectSpread$1({}, text), {}, {
    text: /[^{$}\s~\\]+/
  }),
  namesLiteral: _objectSpread$1(_objectSpread$1({
    and: /\s+[aA][nN][dD]\s+/,
    comma: ',',
    hyphen: '-',
    equals: '='
  ...
  listLiteral: _objectSpread$1(_objectSpread$1({
    and: /\s+and\s+/
  }, text), {}, {
    text: /[^{$}\s~\\]+/
  }),
  separatedLiteral: _objectSpread$1(_objectSpread$1({
    comma: ','
  }, text), {}, {
    text: /[^{$}\s~\\,]+/
  }),
  annotation: _objectSpread$1(_objectSpread$1({}, text), {}, {
    colon: ':',
    equals: '=',
    comma: ',',
    semicolon: ';',
  ...
  bracedLiteral: _objectSpread$1(_objectSpread$1({}, text), {}, {
    rbrace: {
      match: '}',
      pop: true
    },
  ...
  mathLiteral: _objectSpread$1(_objectSpread$1({}, text), {}, {
    mathShift: {
      match: '$',
      pop: true
    },
  ...
  environment: _objectSpread$1(_objectSpread$1({
    commandEnd: {
      match: '\\end',
      pop: true
    }
...
const valueGrammar = new Grammar({
  ...
  Command() {
    ...
    } else if (command in mathCommands) {
      return mathCommands[command];
...
function formatEntryValues({
  ...
  for (const property in properties) {
    ...
    if (annotation) {
      ...
      output.properties[property] = format$4(property, value);
    }
  }
  return output;
}
...
function formatEntry$1(entry, dict) {
  const fields = [];
