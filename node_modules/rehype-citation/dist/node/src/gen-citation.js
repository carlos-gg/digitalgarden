 ...
 */
export const genCitation = (citeproc, mode, entries, citationIdRoot, citationId, citationPre, options, isComposite, citationFormat) => {
    ...
    if (mode === 'note') {
        ...
        return [
            citationText,
            htmlToHast(`<span class="${(inlineClass ?? []).join(' ')}" id=${ids}><sup><a href="#cite-fn-${citationId}" id="cite-fnref-${citationId}" data-footnote-ref aria-describedby="footnote-label">${citationId}</a></sup></span>`),
        ];
    }
        ...
        if (entries.length === 1) {
            ...
            return [
                citationText,
                htmlToHast(`<span class="${(inlineClass ?? []).join(' ')}" id=${ids}>${output.join('')}</span>`),
            ];
        }
    }
    else {
