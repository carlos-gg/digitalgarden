    ...
    for (let i = 0; i < bibliography.length; i++) {
        if (!isValidHttpUrl(bibliography[i], options.path)) {
            ...
            if (options.path) {
                ...
                if (isNode) {
                    bibliography[i] = await import('path').then((path) => path.join(options.path, bibliography[i]));
                ...
                if (isNode) {
                    bibliography[i] = await import('path').then((path) => path.join(file.cwd, bibliography[i]));
...
export const getSortedRelevantRegistryItems = (citeproc, relevantIds, sorted) => {
    ...
    if (sorted) {
        ...
        for (const item of citeproc.registry.reflist) {
            if (relevantIds.includes(item.id))
                res.push(item);
        }
    }
    else {
...
export const isSameAuthor = (item, item2) => {
    ...
    if (authorList.length !== authorList2.length)
        return false;
    for (let i = 0; i < authorList.length; i++) {
        if (authorList[i].family !== authorList2[i].family)
            return false;
    }
    return true;
};
