...
See [[Ensembles, multi-models]], Improving predictions of Ensembles and averaging. How to reduce uncertainty on future predictions? 

Ideas: 
1. Physics-informed FNO (PINO): mass, momentum, energy conversation, emergent constraints, etc.  
2. Temporal FFTs: using the temporal correlations in the data, right now FourCastNet only uses 2D spatial FFTs  
3. Stacks of historical state vectors: moving forward from the current quasi-Markovian approach where we only use the current timestep to predict the next timestep  
4. Extrapolation: a major challenge for ML (physics constraints will help, physics-based renormalization like Pritchard et al.’s recent work is promising)  
5. Two-timing approach: two coupled networks, one that takes larger steps and the other for finescales, with two-way interaction and feedback. This approach will likely be necessary to get to prediction on long horizons (S2S and beyond)
...
### 2022
- SIAM Conference on Imaging Science (IS22). March 22 - 25, 2022
	- Deadline submission: Sept 6 https://www.siam.org/conferences/cm/submissions-and-deadlines/is22-submissions-deadlines
